...
import output_Integration as solver # Imports your group's python script

app = Flask(__name__)
CORS(app) # Permits the browser to communicate with this python server
...
def solve_grid():
    ...
    # FIX: Use /tmp directory for Vercel (Standard filesystem is Read-Only)
    temp_path = os.path.join("/tmp", "temp_grid.xlsx")
    
    file.save(temp_path)
    
    try:
        # 1. Parse Input (Student A)
        net = solver.parse_cdf_excel(temp_path, sheet_name=0) # Reads first sheet
        ...
        # 2. Execute Math Engine (Student B)
        import time
        start_time = time.perf_counter()
        sol = solver.bfs_power_flow(case)
        end_time = time.perf_counter()
        
        # 3. Serialize Output (Student C)
